...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      case "invoke_agent": {
        const { agent_name, request: agentRequest, context, urgency } = InvokeAgentSchema.parse(args);
        ...
        await client.query(
          `INSERT INTO agent_messages (from_agent_id, to_agent_id, type, priority, subject, content, context)
           VALUES ((SELECT id FROM agents LIMIT 1), $1, 'request', $2, $3, $4, $5)`,
          [agent.id, urgency, `Request to ${agent.name}`, agentRequest, JSON.stringify(context || {})]
        );
